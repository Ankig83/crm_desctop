...
class ClientsTab(QWidget):
    def __init__(self, conn: sqlite3.Connection, parent=None) -> None:
        ...
        self._contacts.setToolTip("Для RUS поле «Телефон» покупателя берётся отсюда при экспорте, если отдельный телефон не задан.")
        ...
        for w in (self._ext, self._name, self._inn, self._contact_person, self._email, self._city_region_zip):
    ...
    def reload(self) -> None:
        ...
        for c in clients.list_all(self._conn):
            item = QListWidgetItem(f"{c.name or '(без названия)'} — ИНН {c.inn or '—'}")
